...
### 🌟 Dashboard Features
- **🔍 백테스트 스캐너**: 
  - 원하는 투자금, 기간을 설정하여 전 종목 복리 시뮬레이션 및 인터랙티브 타점(Plotly YYYY-MM-DD 형식) 차트 제공
  - **다양한 자동 청산 전략 (손절/익절) 지원**: `-3%/-5%/-10% 기계적 손절`, `ADX 25 돌파 추세이탈 강제청산`, `20일선 기준 하향 이탈시 즉시 손절` 등을 선택하여 다각도로 백테스트 가능
  - **직관적인 UI/UX**: 순위표 기반 정렬, 단일 행/셀 클릭만으로 모든 해당 종목 상세 내역 연동 노출 및 즉각적인 관심종목 등록 기능 지원
...
- **🧬 시장 국면별 맞춤 전략 (Regime Switching)**: 허스트 지수(Hurst Exponent)를 계산하여 현재 장세가 박스권인지, 강력한 추세장인지 판별합니다.
  - 박스권($H < 0.45$): 낙폭 과대 시 **평균회귀 매수**, 반등 시 익절
  - 강력 추세장($H > 0.55$): 이평선 정배열(MA5 > MA20 > MA60) 진입 시 **추세 탑승 매수**, 추세선(20일선) 이탈 시 청산
